...
const clearCallLock = (userId, consultationId) => {
  if (!userId) return;
  const key = userId.toString();
  const lock = activeCallLocks.get(key);
  if (!lock) return;
  if (consultationId && lock.consultationId !== consultationId.toString()) return;
  ...
  io.on('connection', async (socket) => {
    ...
    socket.on(SOCKET_EVENTS.JOIN_CONSULTATION, async ({ consultationId } = {}, ack) => {
      try {
        ...
        const payload = {
          message: err.message || 'Failed to join consultation',
        };
        socket.emit(SOCKET_EVENTS.ERROR, payload);
        if (typeof ack === 'function') ack({ ok: false, ...payload });
      }
    ...
    socket.on('videoCall:request', async ({ consultationId } = {}) => {
      try {
        ...
        if (!targetUserId) {
          return emitCallError(socket, 'USER_BUSY', 'Call recipient not found');
        }
        ...
        const existingTargetLock = getCallLock(targetUserId);
        if (existingTargetLock) {
          return emitCallError(socket, 'USER_BUSY', 'The other person is busy on another call');
        }
    ...
    socket.on('videoCall:decline', async ({ consultationId } = {}) => {
      try {
        ...
        clearConsultationCallLocks(active.consultation._id);
...
const cleanupTimer = setInterval(() => {
  ...
  for (const [userId, lock] of activeCallLocks.entries()) {
    const ttl = lock.status === 'requested' ? 2 * 60 * 1000 : 2 * 60 * 60 * 1000;
